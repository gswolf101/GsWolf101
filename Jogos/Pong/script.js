...
function startGame(isSinglePlayer) {
    ...
    if (gameScreen && rightLivesContainer && currentScoreDisplay) {
        ...
        // Corrigido: Garantir que scoreGoalsContainer seja visível no single-player
    ...
    if (mysteryBox) {
        ...
        if (collides(ball, box)) {
            ...
            if (lastPlayerTouched) {
                ...
                console.log(`Poder ${power} coletado por ${lastPlayerTouched}`);
            }
            mysteryBox = null;
            mysteryBoxEndTime = 0;
...
System: **mysteryBox** e **update** na função `update` para evitar a interrupção do jogo com alertas.

### Código Atualizado: `script.js`

Abaixo está o código completo do `script.js` com as correções solicitadas. As principais alterações são:

1. **Correção do Placar no Modo Single-Player**:
   - Modificada a função `startGame` para garantir que `#scoreGoalsContainer` seja exibido no modo single-player.
   - Ajustada a lógica para manter a consistência com o CSS atualizado.

2. **Remoção dos Alertas**:
   - Removidas as chamadas `alert` na seção de colisão com a caixa misteriosa na função `update`.
   - Substituídas por mensagens de console para depuração (`console.log`).

3. **Manutenção dos Indicadores de Poder**:
   - Verificado que os indicadores `#leftPower` e `#rightPower` já exibem o texto do poder com as cores corretas (azul para escudo, roxo para raio, vermelho para inversão, verde para crescer), conforme definido no CSS.
